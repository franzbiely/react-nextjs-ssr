...
module.exports = function(app){
    ...
    app.get("/api/getcategory/:slug", (req, res) => {
        ...
        database.query("SELECT * FROM categories WHERE slug='"+slug+"' AND parent_ID IS NULL LIMIT 200",
        (error, result) => {
    ...
    app.get("/api/getmodelsbycategory/:slug/page/:page?", (req, res) => { 
        ...
        INNER JOIN product_heirarchy 	as ph2 ON ph2.parent_ID = ph1.ID \
        INNER JOIN products 			as p ON p.parent_ID = ph2.ID \
        WHERE a.type='brand' AND ph1.type='family' AND ph2.type='series' and c.slug = '${slug}' and c.parent_ID IS NULL LIMIT 20 OFFSET ${pageNum ? pageNum * 20 : 0}`,
        (error, result) => {
            if(error) throw error;
           return res.send(result) 
        })
